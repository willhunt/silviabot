...
TemperatureSensor::TemperatureSensor(int sensor_pin)
    : SilviaPublisher("boiler_temperature", &temperature_msg_, PUB_TEMPERATURE_INTERVAL)
...
double TemperatureSensor::readSensor() {
    return analogRead(sensor_pin_) * TEMPERATURE_SENSOR_COEFFICIENT;
    // return 37.0;
}
...
// PRESSURE SENSOR --------------------------------------------------------------
PressureSensor::PressureSensor(int sensor_pin)
    : SilviaPublisher("boiler_pressure", &pressure_msg_, PUB_TEMPERATURE_INTERVAL)
    , SilviaSensor(FILTER_MOVINGAVERAGE)
    , sensor_pin_(sensor_pin)
{
    pressure_msg_.header.frame_id = "micro";
    pressure_msg_.variance = 0.1;
}

void PressureSensor::setup(NodeHandle* nh) {
    // Set zero affset for gauge pressure
    double reading_sum = 0;
    for (int i = 0; i < 10; i++) {
        reading_sum += readSensor();
        delay(10);
    }
    zero_offset_ = reading_sum / 10;
    SilviaPublisher::setup(nh);
}

double PressureSensor::readSensor() {
    double reading = (analogRead(sensor_pin_) - PRESSURE_SENSOR_OFFSET) * PRESSURE_SENSOR_COEFFICIENT - zero_offset_;
    if (reading < 0) return 0.0;
    else return reading;
}

void PressureSensor::populateMessage() {
    pressure_msg_.header.stamp = nh_->now();
    pressure_msg_.fluid_pressure = reading_last_;
}
...
WaterLevelSensor::WaterLevelSensor(int sensor_pin) 
    : SilviaPublisher("water_level", &water_msg_, PUB_WATERLEVEL_INTERVAL)
    , sensor_pin_(sensor_pin)
{
